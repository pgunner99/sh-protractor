...
Support.prototype.logIntoPriorityQuote = function(user, password, callback){
  browser.get("https://wwwtest.internal.priorityhealth.com/app/priority-quote-beta/#/login?returnUrl=%2Flogin");
  browser.element(sut.by.name("username")).sendKeys(user);
  browser.element(sut.by.name("password")).sendKeys(password);
  browser.ignoreSynchronization = true;
  browser.element(sut.by.css("input.ph-btn.btn-green")).click();
  browser.sleep(5000).then(function(result) {
    callback(result)
  });
};
